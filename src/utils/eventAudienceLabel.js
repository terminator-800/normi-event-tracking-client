...
function audienceRowDepartmentCode(audience) {
  if (!audience || typeof audience !== "object") return null;
...
function audienceRowMajor(audience) {
...
function audienceRowYearLevelRaw(audience) {
  if (!audience || typeof audience !== "object") return null;
...
export function formatAudienceYearLevel(yl) {
...
export function abbrevBsedMajorLabel(rawMajor) {
...
export function abbrevBsbaMajorLabel(rawMajor) {
  const major = String(rawMajor ?? "").trim().toLowerCase();
...
function audienceRowCourseName(a) {
...
function inferBsbaMajorFromCourseName(a) {
...
function bsbaMajorForAudienceLabel(a) {
...
export function normalizeEvAudiences(raw) {
  if (raw == null) return [];
 ...
 */
function labelOneAudienceRow(a) {
...
function appendYearLevelSuffix(baseLabel, audience) {
 ...
 */
export function getAudienceScopeLabel(ev) {
  if (!ev) return "—";
  const instituteWide =
  ...
  if (rows.length > 0) {
    ...
    if (allBsed) {
      return appendYearLevelSuffix("BSED", rows[0]);
    }
