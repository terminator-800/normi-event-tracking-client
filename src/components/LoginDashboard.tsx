...
import { getApiErrorMessage, isApiAxiosError, type AuthSession } from "../types/api";

type LoginDashboardProps = {
  onLoginSuccess: (data: AuthSession | null | undefined) => void;
};

export default function LoginDashboard({ onLoginSuccess }: LoginDashboardProps) {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    signIn(
      ...
      {
        onError: (err) => {
          if (isApiAxiosError(err) && err.response?.status === 429) {
