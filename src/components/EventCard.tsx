...
type EventCardVariant = "default" | "minimal" | "modalHorizontal";
...
type AllDepartmentCourseNamesListProps = {
  className?: string;
};
...
type MandatoryYesNoProps = {
  ev: DisplayEvent;
  compact?: boolean;
};

type EventCardMinimalProps = {
  ev: DisplayEvent;
};

type EventCardModalHorizontalProps = {
  ev: DisplayEvent;
};
...
function allDepartmentCourseLabel(course: DepartmentCourse): string {
  return course.name;
}
...
function AllDepartmentCourseNamesList({ className = "" }: AllDepartmentCourseNamesListProps) {
  return (
...
function Row({ label, value, small = false }: RowProps) {
  return (
...
function graceMins(v: number | string | null | undefined): string {
  if (v == null || v === "") return "—";
  const n = Number(v);
 ...
 */
function parseScheduleSlots(timeSlots: string | null | undefined): string[] {
...
function parsePeriodRangeFromTimeSlots(
  timeSlots: string | null | undefined,
  period: "AM" | "PM",
): TimeRange {
...
function resolveTimePair(ev: DisplayEvent, period: "AM" | "PM"): TimeRange {
  ...
  const inRaw = isAm ? ev.am_time_in : ev.pm_time_in;
  const outRaw = isAm ? ev.am_time_out : ev.pm_time_out;
...
function shouldShowPeriodFromDuration(
...
function hasVisibleRange(range: TimeRange | null | undefined): boolean {
  if (!range) return false;
  return range.start !== "—" || range.end !== "—";
}

function ScheduleLine({ line }: ScheduleLineProps) {
...
function ScheduleBlocks({ timeSlots, compact = false }: ScheduleBlocksProps) {
...
function MiniCell({ label, children, className = "", dense = false }: MiniCellProps) {
  return (
...
function MandatoryYesNo({ ev, compact = false }: MandatoryYesNoProps) {
...
function EventCardMinimal({ ev }: EventCardMinimalProps) {
        ...
        <div className="mt-3 border-t border-[#36454F]/10 pt-3">
          ...
          <div className="flex flex-wrap gap-2">
            {ev.is_all_departments
              ...
              : (ev.audiences as AudienceRow[]).map((a, idx) => (
                  <div
                    key={idx}
                  ...
                  >
                    ...
                    {a.course_code ? ` · ${String(a.course_code)}` : ""}
...
function EventCardModalHorizontal({ ev }: EventCardModalHorizontalProps) {
  const audience = ev.is_all_departments ? "All departments" : getAudienceScopeLabel(ev);

  const amRange = resolveTimePair(ev, "AM");
  const pmRange = resolveTimePair(ev, "PM");
  const sessionDescriptor = ev.duration;
            ...
            <div className="mt-2.5 grid grid-cols-1 gap-2 sm:grid-cols-2">
                {(ev.audiences as AudienceRow[]).map((a, idx) => (
                  <div key={idx} className="rounded-lg border border-[#07713c]/20 bg-[#f6fff8] p-3 text-xs space-y-1.5">
                    <Row label="Department" small value={String(a.department_name ?? "—")} />
                    <Row label="Course code" small value={String(a.course_code ?? "—")} />
                    <Row label="Course name" small value={String(a.course_name ?? "—")} />
                    <Row
                      label="Year level"
                      small
                      value={a.year_level != null ? formatAudienceYearLevel(a.year_level) : "—"}
                    />
                  </div>
                ))}
            </div>
...
export default function EventCard({ event: ev, variant = "default" }: EventCardProps) {
  if (variant === "minimal") {
