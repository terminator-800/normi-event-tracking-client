...
function periodStatusLabel(status: AcademicPeriodRecord["status"]): string {
...
export default function AcademicSettingsPage({ onNavigate, onLogout }: DeskPageProps) {
  ...
  const { data: periods = [], isLoading, refetch } = useAcademicPeriodsList(canViewPeriods);
  ...
  useEffect(() => {
    if (canViewPeriods) return;
    onNavigate?.("events");
  }, [canViewPeriods, onNavigate]);

  if (!canViewPeriods) return null;
                    ...
                    {paginatedPeriods.map((period) => {
                      ...
                      return (
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <span
                            className={`inline-flex rounded-full px-3 py-1 text-sm font-semibold ${statusBadgeClass(period.status)}`}
                          >
                          ...
                          {period.status === "active" ? (
                            ...
                            <span className="text-sm font-medium text-black/50">
                              {period.status === "draft" ? "Draft" : "Archived"}
                            </span>
                          )}
                        </div>
